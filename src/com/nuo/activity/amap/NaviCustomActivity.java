...
public class NaviCustomActivity extends Activity implements
	...
	// 导航可以设置的参数
	private boolean mDayNightFlag = AMapUtil.DAY_MODE;// 默认为白天模式
	private boolean mDeviationFlag = AMapUtil.YES_MODE;// 默认进行偏航重算
	private boolean mJamFlag = AMapUtil.YES_MODE;// 默认进行拥堵重算
	private boolean mTrafficFlag = AMapUtil.OPEN_MODE;// 默认进行交通播报
	private boolean mCameraFlag = AMapUtil.OPEN_MODE;// 默认进行摄像头播报
	private boolean mScreenFlag = AMapUtil.YES_MODE;// 默认是屏幕常亮
	// 导航界面风格
	...
	public void onNaviSetting() {
		...
		bundle.putInt(AMapUtil.THEME, mThemeStle);
	...
	private void processBundle(Bundle bundle) {
		...
		if (bundle != null) {
			mDayNightFlag = bundle.getBoolean(AMapUtil.DAY_NIGHT_MODE,
					mDayNightFlag);
			mDeviationFlag = bundle.getBoolean(AMapUtil.DEVIATION, mDeviationFlag);
			mJamFlag = bundle.getBoolean(AMapUtil.JAM, mJamFlag);
			mTrafficFlag = bundle.getBoolean(AMapUtil.TRAFFIC, mTrafficFlag);
			mCameraFlag = bundle.getBoolean(AMapUtil.CAMERA, mCameraFlag);
			mScreenFlag = bundle.getBoolean(AMapUtil.SCREEN, mScreenFlag);
			mThemeStle = bundle.getInt(AMapUtil.THEME);
